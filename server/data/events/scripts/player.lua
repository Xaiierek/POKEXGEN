	...
	if self:getGroup():getAccess() then
		...
		if houseId and ItemType(thing.itemid):getName():find("door") then
			...
			if house then
				...
				--print("Sending ExtendedOpcode: " .. house_information) -- Dodaj tę linię, aby wyświetlić wysyłane dane w konsoli TFS
...
function Player:onExtendedOpcode(opcode, buffer)
    if opcode == 101 then
        ...
        local npcName = split[1]
        local text = split[2]
        self:sendTextMessage(MESSAGE_EVENT_ADVANCE, npcName .. " mówi: " .. text)
    end
    return true
end

function Player:onRecvbyte(opcode, buffer)
    return onRecvbyte(self, opcode, buffer) -- Właściwie wywołujemy funkcję z "vocationinfo.lua"
end