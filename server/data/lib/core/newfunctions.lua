...
function Position:createFlyFloor()
    ...
    local toTile = Tile(pose)
    if not toTile or (not toTile:getGround() and not toTile:getItemById(flyFloor)) then
		doAreaCombatHealth(0, 0, pose, 0, 0, 0, CONST_ME_POFF)
		Game.createItem(flyFloor, 1, pose)
		end
		end
	end
end
...
function Player:canFlyUp()
	...
	--flyfloor:remove(fromPosition.z);
