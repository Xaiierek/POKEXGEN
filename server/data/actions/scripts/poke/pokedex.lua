...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target:isCreature() then -- check if it is on dex or add to dex
		...
		if player:getStorageValue(storage) == -1 then -- add to dex	
			...
			local dex_information = string.format("dex_data|%s|%s|%s", monsterType, portraitId, portraitItemType)
			print("Sending ExtendedOpcode: " .. dex_information .. target:getSummonStatus())
			player:sendExtendedOpcode(198, dex_information .. "|" .. target:getSummonStatus())
		end
	end
	return true
end
