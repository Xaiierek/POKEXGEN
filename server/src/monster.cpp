...
bool Monster::canWalkTo(Position pos, Direction direction) const
{
	...
	if (isInSpawnRange(pos)) {
		...
		Tile* tile = g_game.map.getTile(pos);
		if (tile) {
			// Check if the target tile has the TILESTATE_NO_MONSTER_MOVEMENT flag
			if (tile->hasFlag(TILESTATE_NO_MONSTERMOVE)) {
				return false;
			}

			if (tile->getTopVisibleCreature(this) == nullptr && tile->queryAdd(0, *this, 1, FLAG_PATHFINDING) == RETURNVALUE_NOERROR) {
				return true;
			}
		}
	}
